...
from dataset.video_dataset import FaceVideoDataset
...
p.add_argument('--animated', type=int, default=-1, required=False,
               help='Animation code dimension to use.')
...
def train():
    ...
    if opt.sample_observations_train == [-1]:
        tot_obsevations = list(range(_NUM_OBSERVATIONS))
        bin_size = len(tot_obsevations) // 10
        sample_frames = list(np.random.randint(bin_size, size=10) + np.arange(10) * bin_size)
        opt.sample_observations_train = random.choices(
            [x for x in tot_obsevations if x not in sample_frames], k=50) + sample_frames
        opt.sample_observations_val = [x for x in tot_obsevations if x not in opt.sample_observations_train]

        print('> Train observations: ', opt.sample_observations_train)
        print('> Sample frames: ', sample_frames)
        print('> Val observations: ', opt.sample_observations_val)

    train_dataset = FaceVideoDataset(
    ...
    for img_sidelength, max_steps, batch_size in zip(img_sidelengths, max_steps_per_sidelength,
        ...
        while True:
            for model_input, ground_truth in train_dataloader:
                ...
                writer.add_scalar(
                    "Loss/scaled_regularization_loss", weighted_reg_loss, iter)
