...
def plot_camera_scene(cam_pose):
    ...
    def _plot_cameras(ax, cam_pose, color: str = "blue"):
        """
        Plots a set of `cameras` objects into the maplotlib axis `ax` with
        color `color`.
        """
        cam_wires_canonical = _get_camera_wireframe().T
        cam_wires_canonical = np.expand_dims(cam_wires_canonical, axis=0)
    ...
    ax.clear()
    handle_cam = _plot_cameras(ax, cam_pose, color="#FF7D1E")
    plot_radius = 5
