...
_CAM_CENTER = (0., 0.11, 1.1)
_OBJ_CENTER = (0., 0.11, 0.1)
...
class SRNsModel(nn.Module):
    def __init__(self,
                 num_instances,
                 # int: full code, tuple (identity, attribute)
                 latent_dim,
                 tracing_steps,
                 hidden_dim=256,
                 feat_dim=None,
        ...
        if use_encoder:
            ...
            # print('*** INIT Embedding: latent_code = (%dx%d), ami_code = (%dx%d)' %
            #       (self.num_instances, self.latent_dim-self.animated, len(self.sample_frames), self.animated))

            # self.get_embedding = lambda x: torch.cat([
            #     self.latent_codes(x['instance_idx'].long().cuda()),
            #     self.ami_code(x['observation_idx'].long().cuda())], axis=1)
    ...
    def forward(self, pose, z, intrinsics, uv, dpt=None, device=None, auc_input=None):
